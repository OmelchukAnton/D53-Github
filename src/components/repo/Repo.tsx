...
import { AvatarImage, RepoItem, RepoHeader, RepoHeaderName, RepoHeaderIcons } from "./styles";
...
export const Repo: React.FC<{repo: RepoType}> = (props) => {
    const {repo} = props;
        ...
        <RepoItem>
            <RepoHeader>
                <RepoHeaderName>
                    ...
                    <NavLink to={`/repo/${repo.owner.login}/${repo.name}`}>{repo.name}</NavLink>
                </RepoHeaderName>
                <RepoHeaderIcons>
            ...
            <div className="repo-last-commit">Last commit: {formatDate(repo.updated_at)}</div>
