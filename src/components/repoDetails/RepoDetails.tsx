...
import {getContributors, getCurrentRepo, getIssues, getReadme} from "../api/api";
import { Section, CardItem} from "./styles";
import {CallbackType, ContributorType, HandlerFuncType, IssueType, ReadmeType, RepoType} from "../../types";
        ...
        <div className="site-page-header-ghost-wrapper">
            ...
            >
                <Descriptions size="small" column={3}>
                    <Descriptions.Item label="Language">{repo.language}</Descriptions.Item>
                    <Descriptions.Item label="Creation Time">{formatDate(repo.created_at)}</Descriptions.Item>
                    <Descriptions.Item label="Last update">{formatDate(repo.updated_at)}</Descriptions.Item>
                </Descriptions>
            </PageHeader>
            ...
            <Collapse>
                <Panel header="Issues" key="1">
                    <List
                        ...
                        bordered
                        dataSource={issues}
                        renderItem={(issue: IssueType) => <List.Item>
                            <a href={issue.html_url} target="_blank" rel="noreferrer">{issue.title}</a>
                        </List.Item>}
                    />
                </Panel>
                {readme && (<Panel header="README.md" key="2">
